...
def dmg_roll_super():
    out = 0
    for i in range(200):
        out += np.random.randint(1,high=20)
    return out
    ...
    if message.content.startswith('attac'):
        if message.author.name not in attacked_userlist:
            ...
            if message.author.name=='Xanxus85':
                ...
                attacked_userlist.append(message.author.name)
            if atk<bumpkin_AC:
                out_msg = "{} {}\n**Attack Roll:** {}\nYou Missed!".format(message.author.mention,bumpkin_user.mention,atk)
                ...
                if (bumpkinhealth <= 0):
                    bumpkinhealth = 0
                    os.environ['bumpkin_health'] = str(bumpkinhealth)
                    out_msg = "{} {}\n**Attack Roll:** {}\nYou hit for {} damage and killed bumpkin!".format(message.author.mention,bumpkin_user.mention,atk,dmg)
                    await message.channel.send(content=out_msg)
                    ...
                    os.environ['bumpkin_health'] = str(bumpkinhealth)
                    out_msg = "{} {}\n**Attack Roll:** {}\nYou hit for {} damage, bumpkin has {}/{} hp left!".format(message.author.mention,bumpkin_user.mention,atk,dmg,bumpkinhealth,bumpkin_max)
                    await message.channel.send(content=out_msg)
                    print('bumpkin attacked by {} for {}dmg and now has {}hp left'.format(message.author,dmg,bumpkinhealth))
        else:
            await message.channel.send(content="{} {}\nYou already rolled today!".format(message.author.mention,bumpkin_user.mention))
    #hello            
